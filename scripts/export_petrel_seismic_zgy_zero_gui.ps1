...
$pythonScript = Join-Path $scriptDir "export_petrel_seismic_zgy_zero_gui.py"
...
if (-not (Test-Path -LiteralPath $pythonScript -PathType Leaf)) {
    throw "Zero-GUI ZGY seismic exporter not found: $pythonScript"
}
...
$pyArgs = @('--export-package', $ExportPackage)
if ($ExportVolume) { $pyArgs += '--export-volume' }
if ($NoSlices) { $pyArgs += '--no-slices' }
