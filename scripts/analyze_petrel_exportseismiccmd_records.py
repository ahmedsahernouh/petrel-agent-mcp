#!/usr/bin/env python3
"""Map saved ExportSeismicCmd records in the Petrel native workflow store."""
        ...
        records.append(
            {
                ...
                "context": printable_context(data, command_offset - 220, 900),
            }
        )
...
def main() -> int:
    ...
    report_dir = Path(args.output_root).resolve() / f"exportseismiccmd_record_map_{stamp}"
    report_dir.mkdir(parents=True, exist_ok=True)
    records_csv = report_dir / "exportseismiccmd_records.csv"
    tokens_csv = report_dir / "exportseismiccmd_token_hits.csv"
    model_csv = report_dir / "exportseismiccmd_model_hits.csv"
    report_json = report_dir / "exportseismiccmd_record_map.json"
    summary_md = report_dir / "exportseismiccmd_record_map_summary.md"
    ...
    lines = [
        ...
        f"- ExportSeismicCmd records: {len(records)}",
        "",
        "## Records",
        "",
    ...
    for row in token_rows:
        if row["term"] in {"_donor.sgy", "sgy2", "sgy3", ".sgy", "segy"}:
    ...
    print("ExportSeismicCmd record map complete")
